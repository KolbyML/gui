...
void TestImportDescriptors(interfaces::Node& node)
{
    ...
    std::shared_ptr<CWallet> w1 = std::make_shared<CWallet>(node.context()->chain.get(), "", gArgs, CreateMockWalletDatabase());
    w1->LoadWallet();
    ...
    const std::shared_ptr<CWallet> wpriv = std::make_shared<CWallet>(node.context()->chain.get(), "", gArgs, CreateMockWalletDatabase());
    wpriv->LoadWallet();
