...
void TestImportMulti(interfaces::Node& node)
{
    ...
    auto get_multisig = [&wallet](int key_length = 3) {
        Key key;
        CKey k[3];
        for (int i = 0; i < 3; i++)
            k[i].MakeNewKey(true);
        ...
        // privkey and pubkey here are a list of keys seperated by commas
    ...
    auto get_unspent_info = [&wallet](QString& input) {
        ...
        std::vector<COutput> vecOutputs;
        {
            ...
            LOCK(wallet->cs_wallet);
            vecOutputs = AvailableCoinsListUnspent(*wallet, &cctl, 0, MAX_MONEY, MAX_MONEY, 0).all();
        }
    ...
    int64_t timestamp = node.context()->chainman->ActiveChain().Tip()->GetMedianTimePast();
    int64_t watchonly_timestamp = timestamp;
