...
static std::string
RecurseImportData(const CScript &script, ImportData &import_data, const ScriptContext script_ctx) {
...
bool ProcessDescriptorImport(CWallet &wallet, const ImportDescriptorData &data, std::vector<std::string> &warnings,
    ...
    WalletDescriptor w_desc(std::move(data.parsed_desc), timestamp, data.range_start, data.range_end,
                            data.next_index);
...
bool ProcessPublicKey(CWallet &wallet, std::string strLabel, CPubKey pubKey) {
    ...
    wallet.MarkDirty();
    wallet.ImportScriptPubKeys(strLabel, script_pub_keys, true, true, 1);
    wallet.ImportPubKeys({pubKey.GetID()}, {{pubKey.GetID(), pubKey}}, {}, false, false, 1);
    return true;
}
...
bool ProcessPrivateKey(CWallet &wallet, std::string strLabel, CKey key) {
    ...
    if (pubkey.IsCompressed()) {
        wallet.ImportScripts({GetScriptForDestination(WitnessV0KeyHash(vchAddress))}, 0 /* timestamp */);
    }
    return true;
}
...
bool ProcessAddress(CWallet &wallet, std::string strAddress, std::string strLabel, bool fP2SH) {
    ...
    if (IsValidDestination(dest)) {
        ...
        wallet.MarkDirty();
        wallet.ImportScriptPubKeys(strLabel, {GetScriptForDestination(dest)}, false /* have_solving_data */,
        ...
        wallet.ImportScripts(scripts, 0 /* timestamp */);

        if (fP2SH) {
            scripts.insert(GetScriptForDestination(ScriptHash(redeem_script)));
        }

        wallet.ImportScriptPubKeys(strLabel, scripts, false /* have_solving_data */,
                                    true /* apply_label */, 1 /* timestamp */);
    } else {
