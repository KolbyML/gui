        ...
        for (const UniValue& data : requests.getValues()) {
            ...
            if (warnings.size()) {
                ...
                for (const auto& w: warnings)
                    warning.push_back(w);
                result.pushKV("warnings", warning);
            }
    ...
    UniValue response(UniValue::VARR);
    {
        ...
        for (const UniValue& request : requests.getValues()) {
            ...
            try {
                ...
                descriptor_data.label = request.exists("label") ? request["label"].get_str() : "";
                ...
                if (!descriptor_data.parsed_desc->IsRange() && request.exists("range")) {
                    ...
                    if (request.exists("range")) {
                        ...
                        descriptor_data.range_end = gArgs.GetIntArg("-keypool", DEFAULT_KEYPOOL_SIZE);
                    }
